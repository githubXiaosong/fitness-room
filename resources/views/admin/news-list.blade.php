...
<div class="x-body">
    ...
    <xblock>
        ...
        <span class="x-right" style="line-height:40px">共有数据：{{ count($news) }} 条</span>
    </xblock>
        ...
        @foreach($news as $item)
            <tr>
                ...
                <td><img  class="img-circle img-square"
                         src="{{ '/storage/'.$item->cover_uri }}"></td>

                <td>{{ $item->created_at }}</td>
                <td class="td-status">
                    <span class="layui-btn {{$item->status==STATUS_NORMAL?:'layui-btn-normal'}} layui-btn-mini">{{ $item->status==STATUS_OFF_NORMAL?'未启用':'已启用'}}</span>
                </td>

                <td class="td-manage">
                    <a href="#" onclick="layer.confirm('确认要停用/启用吗？',function(index){
                            submit_as_form('{{url('admin/api/changeNewsStatus')}}','news_id','{{ $item->id }}')
                            });" title="{{ $item->status==STATUS_NORMAL?'停用':'启用' }}">
                        <i class="layui-icon">{{$item->status==STATUS_NORMAL?'&#xe601;':'&#xe62f;'}} </i>
                    </a>

                    <a title="删除" href="#" onclick="layer.confirm('确认要删除吗？',function(index){
                            submit_as_form('{{url('admin/api/deleteNews')}}','news_id','{{ $item->id }}')
                            });">
