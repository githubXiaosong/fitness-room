...
<div class="x-body">
    ...
    <form class="layui-form" action="{{ url('admin/api/addProduct') }}" method="post" enctype="multipart/form-data">
        {{ csrf_field() }}
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-inline  {{ $errors->has('url') ? 'error-input' : '' }}">
                <input type="text" name="url" required="" id="url"
                       autocomplete="off" class="layui-input" value="{{ old('url') }}" autofocus>
            </div>
            @if ($errors->has('url'))
                <span class="help-block error-text">
